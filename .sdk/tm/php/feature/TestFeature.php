...
// TaiwanLegalAi SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class TaiwanLegalAiTestFeature extends TaiwanLegalAiBaseFeature
{
    ...
    public function init(TaiwanLegalAiContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (TaiwanLegalAiContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = TaiwanLegalAiParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (TaiwanLegalAiContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(TaiwanLegalAiContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = TaiwanLegalAiParam::call($ctx, $k);
