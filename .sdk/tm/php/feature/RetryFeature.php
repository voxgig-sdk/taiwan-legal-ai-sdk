...
// TaiwanLegalAi SDK retry feature
...
class TaiwanLegalAiRetryFeature extends TaiwanLegalAiBaseFeature
{
    ...
    public function init(TaiwanLegalAiContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TaiwanLegalAiContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TaiwanLegalAiContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TaiwanLegalAiContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
