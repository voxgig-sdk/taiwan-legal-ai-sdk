...
module TaiwanLegalAiUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TaiwanLegalAiResponse.new({})
    ...
    if fetch_err
      ...
      response = TaiwanLegalAiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TaiwanLegalAiResponse.new(fetched)
    else
