...
import (
	...
	omni "github.com/voxgig-sdk/taiwan-legal-ai-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TaiwanLegalAiError); is {
				out := map[string]any{
					"name":    "TaiwanLegalAiError",
...
func omniCtx(arg any, client *sdk.TaiwanLegalAiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TaiwanLegalAiSDK); is {
			client = live
