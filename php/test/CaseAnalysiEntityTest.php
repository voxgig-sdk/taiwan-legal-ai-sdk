...
class CaseAnalysiEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = TaiwanLegalAiSDK::test(null, null);
        $ent = $testsdk->CaseAnalysi(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = case_analysi_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "case_analysi." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        // CREATE
        $case_analysi_ref01_ent = $client->CaseAnalysi(null);
...
function case_analysi_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/case_analysi/CaseAnalysiTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["case_analysi01", "case_analysi02", "case_analysi03"] as $k) {
