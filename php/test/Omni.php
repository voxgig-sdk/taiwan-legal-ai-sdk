...
//    only - a typed TaiwanLegalAiContext there would make every ctx
...
final class TaiwanLegalAiOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TaiwanLegalAiContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new TaiwanLegalAiSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new TaiwanLegalAiResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new TaiwanLegalAiError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new TaiwanLegalAiResponse($clean['response']);
    ...
    public static function err_from_map($m): ?TaiwanLegalAiError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TaiwanLegalAiError(is_string($code) ? $code : '', $msg);
    }
