...
// TaiwanLegalAi SDK feature test
//
...
class FtCtrl extends TaiwanLegalAiControl
{
...
class FtClient
{
    ...
    private TaiwanLegalAiUtility $utility;

    public function __construct(TaiwanLegalAiUtility $utility, array $options)
    {
    ...
    public function get_utility(): TaiwanLegalAiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TaiwanLegalAiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TaiwanLegalAiContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TaiwanLegalAiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TaiwanLegalAiConfig::make_feature($name);
    ...
    private static function buildUrl(TaiwanLegalAiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TaiwanLegalAiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TaiwanLegalAiResponse($fetched);
        }
    ...
    private function fail(TaiwanLegalAiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TaiwanLegalAiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
