...
class TaiwanLegalAiTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TaiwanLegalAiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TaiwanLegalAiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TaiwanLegalAiTestRunner.getenv("TAIWAN_LEGAL_AI_TEST_LIVE")
        override = TaiwanLegalAiTestRunner.getenv("TAIWAN_LEGAL_AI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TaiwanLegalAiTestRunner.getenv(key)
        ...
        explain = TaiwanLegalAiTestRunner.getenv("TAIWAN_LEGAL_AI_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if TaiwanLegalAiTestRunner._test_control is not None:
            return TaiwanLegalAiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TaiwanLegalAiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TaiwanLegalAiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TaiwanLegalAiTestRunner.load_test_control()
...
def load_env_local():
    TaiwanLegalAiTestRunner.load_env_local()


def env_override(m):
    return TaiwanLegalAiTestRunner.env_override(m)


def entity_data(v):
    return TaiwanLegalAiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TaiwanLegalAiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TaiwanLegalAiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TaiwanLegalAiTestRunner.load_test_control()


def live_client_options():
    return TaiwanLegalAiTestRunner.live_client_options()


def live_delay_ms():
    return TaiwanLegalAiTestRunner.live_delay_ms()
