    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TAIWAN_LEGAL_AI_TEST_CONTRACT_SERVICE_ENTID JSON to run live")
        client = setup["client"]
        ...
        contract_service_ref01_data = helpers.to_map(runner.entity_data(contract_service_ref01_ent.create(contract_service_ref01_data, None)))
...
def _contract_service_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "TAIWAN_LEGAL_AI_TEST_CONTRACT_SERVICE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("TAIWAN_LEGAL_AI_TEST_CONTRACT_SERVICE_ENTID"))
