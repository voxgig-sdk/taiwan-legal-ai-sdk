# TaiwanLegalAi Lua SDK

The Lua SDK for the TaiwanLegalAi API. Provides an entity-oriented interface using Lua conventions.
...
```lua
-- Create
local created, _ = client:CaseAnalysi(nil):create({ name = "Example" }, nil)
...
local result, err = client:TaiwanLegalAi(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── taiwan-legal-ai_sdk.lua    -- Main SDK module
...
The main module (`taiwan-legal-ai_sdk`) exports the SDK constructor
