...
describe("ContractServiceEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:ContractService(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = contract_service_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "contract_service." .. _op, _live and "live" or "unit")
    ...
    -- CREATE
    local contract_service_ref01_ent = client:ContractService(nil)
    local contract_service_ref01_data = helpers.to_map(vs.getprop(
...
function contract_service_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/contract_service/ContractServiceTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read contract_service test data: " .. entity_data_file)
  end
