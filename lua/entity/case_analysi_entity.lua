...
local CaseAnalysiEntity = {}
CaseAnalysiEntity.__index = CaseAnalysiEntity


function CaseAnalysiEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CaseAnalysiEntity)
  self._name = "case_analysi"
  self._client = client
...
function CaseAnalysiEntity:get_name()
  return self._name
end


function CaseAnalysiEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CaseAnalysiEntity.new(self._client, opts)
end
...
function CaseAnalysiEntity:mark_deleted()
  self._deleted = true
end


function CaseAnalysiEntity:deleted()
  return true == self._deleted
end


function CaseAnalysiEntity:data_set(args)
  if args ~= nil then
...
function CaseAnalysiEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CaseAnalysiEntity:match_set(args)
  if args ~= nil then
...
function CaseAnalysiEntity:match_get()
...
function CaseAnalysiEntity:stream(action, args, callopts)
...
function CaseAnalysiEntity:_run_op(ctx, post_done)
  local utility = self._utility
