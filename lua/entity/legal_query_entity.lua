...
local LegalQueryEntity = {}
LegalQueryEntity.__index = LegalQueryEntity


function LegalQueryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LegalQueryEntity)
  self._name = "legal_query"
  self._client = client
...
function LegalQueryEntity:get_name()
  return self._name
end


function LegalQueryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LegalQueryEntity.new(self._client, opts)
end
...
function LegalQueryEntity:mark_deleted()
  self._deleted = true
end


function LegalQueryEntity:deleted()
  return true == self._deleted
end


function LegalQueryEntity:data_set(args)
  if args ~= nil then
...
function LegalQueryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LegalQueryEntity:match_set(args)
  if args ~= nil then
...
function LegalQueryEntity:match_get()
...
function LegalQueryEntity:stream(action, args, callopts)
...
function LegalQueryEntity:_run_op(ctx, post_done)
  local utility = self._utility
