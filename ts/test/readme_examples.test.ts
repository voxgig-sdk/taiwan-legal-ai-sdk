...
//     (\`new TaiwanLegalAiSDK(...)\` / \`TaiwanLegalAiSDK.test(...)\` become
//     \`TaiwanLegalAiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'TaiwanLegalAiSDK'
...
const TEST_SEED = {"entity":{"case_analysi":{"test01":{"id":"test01"}},"contract_service":{"test01":{"id":"test01"}},"legal_query":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TaiwanLegalAiSDK(...)\` and any
// \`TaiwanLegalAiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TaiwanLegalAiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TaiwanLegalAiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
