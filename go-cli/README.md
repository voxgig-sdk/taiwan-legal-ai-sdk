...
boru-driven command-line client **and** interactive REPL for the TaiwanLegalAi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/taiwan-legal-ai-cli)
make build

# 2. See usage (words, entities, env vars)
./taiwan-legal-ai-cli --help
...
TAIWAN_LEGAL_AI_BASE=https://api.example.com ./taiwan-legal-ai-cli --help

# 6. No arguments -> interactive REPL
./taiwan-legal-ai-cli
taiwan-legal-ai> /help
taiwan-legal-ai> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/taiwan-legal-ai-cli
   ```
...
Run with no arguments to open a REPL (prompt `taiwan-legal-ai>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `case_analysi`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/taiwan-legal-ai-cli`. |
...
(`./taiwan-legal-ai-cli <expr>`) and interactively (the REPL), and expressions compose the
