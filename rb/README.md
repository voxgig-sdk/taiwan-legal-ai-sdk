...
The Ruby SDK for the TaiwanLegalAi API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
# Create
created, _ = client.CaseAnalysi(nil).create({ "name" => "Example" }, nil)
...
result, err = client.TaiwanLegalAi(nil).load(
  { "id" => "test01" }, nil
)
