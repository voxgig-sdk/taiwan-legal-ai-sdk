...
class ContractServiceEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TAIWAN_LEGAL_AI_TEST_CONTRACT_SERVICE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    contract_service_ref01_data = Helpers.to_map(contract_service_ref01_data_result.respond_to?(:data_get) ? contract_service_ref01_data_result.data_get : contract_service_ref01_data_result)
...
def contract_service_basic_setup(extra)
  ...
  entid_env_raw = ENV["TAIWAN_LEGAL_AI_TEST_CONTRACT_SERVICE_ENTID"]
