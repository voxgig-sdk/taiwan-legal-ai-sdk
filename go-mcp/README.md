...
[MCP](https://modelcontextprotocol.io) server exposing the TaiwanLegalAi SDK as
two agent tools — `taiwan-legal-ai_list` and `taiwan-legal-ai_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/taiwan-legal-ai-mcp)
make build

# 2. Provide credentials via the environment
export TAIWAN_LEGAL_AI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user taiwan-legal-ai \
  -- /absolute/path/to/taiwan-legal-ai-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./taiwan-legal-ai-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// taiwan-legal-ai_list: first page of records
{ "entity": "case_analysi" }
{ "entity": "case_analysi", "query": { } }

// taiwan-legal-ai_load: one record by id
{ "entity": "case_analysi", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/taiwan-legal-ai-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user taiwan-legal-ai \
     -- "$PWD"/dist/*/taiwan-legal-ai-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `taiwan-legal-ai_list` and `taiwan-legal-ai_load` tools now appear
   in new sessions. Ask the agent to *"list case_analysi using taiwan-legal-ai"*
   and it calls `taiwan-legal-ai_list` with `{"entity":"case_analysi"}`.
...
```sh
export TAIWAN_LEGAL_AI_APIKEY=sk_live_xxx            # API key
export TAIWAN_LEGAL_AI_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `taiwan-legal-ai_list` tool
...
### Call the `taiwan-legal-ai_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `taiwan-legal-ai_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `taiwan-legal-ai_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TAIWAN_LEGAL_AI_APIKEY` | API key sent with every request. |
| `TAIWAN_LEGAL_AI_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

case_analysi | contract_service | legal_query

### Smoke test via HTTP (raw JSON-RPC)

```sh
./taiwan-legal-ai-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"taiwan-legal-ai_load","arguments":{"entity":"case_analysi","query":{"id":1}}}}'
```
