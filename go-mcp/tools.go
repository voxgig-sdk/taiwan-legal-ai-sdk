...
type Args struct {
	Entity string         `json:"entity" jsonschema:"case_analysi | contract_service | legal_query"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.TaiwanLegalAiSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "taiwan-legal-ai_list",
		Description: "List records from TaiwanLegalAi. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "taiwan-legal-ai_load",
		Description: "Load a single record from TaiwanLegalAi. " +
...
func runOp(client *sdk.TaiwanLegalAiSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.TaiwanLegalAiSDK, name string) (sdk.TaiwanLegalAiEntity, error) {
	switch strings.ToLower(name) {
	case "case_analysi":
		return client.CaseAnalysi(nil), nil
	case "contract_service":
		return client.ContractService(nil), nil
	case "legal_query":
		return client.LegalQuery(nil), nil
